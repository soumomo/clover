...
if input_mode == "Curated Site & Benchmark Presets":
    ...
    with st.spinner("Processing satellite imagery with Apple M4 GPU acceleration..."):
    ...
    if active_plain_image is not None:
        with st.spinner("Processing pasted image with adaptive multi-scale pyramid on Apple M4 GPU..."):
...
if results.get("is_geotiff") and results.get("wgs_bounds") is not None:
    ...
    st.markdown(
        ...
        f"Processed {metrics.tree_count} mature stems across {metrics.aoi_area_ha:.2f} ha in {results['elapsed_sec']:.2f}s on Apple Silicon M4 GPU. "
    ...
    if "pil_image" in results:
        ...
        st.markdown(
            ...
            f"Delineated <b>{metrics.tree_count} individual crowns</b> across <b>{metrics.aoi_area_ha:.2f} ha</b> in {results['elapsed_sec']:.2f}s on Apple Silicon M4 GPU.<br/>"
